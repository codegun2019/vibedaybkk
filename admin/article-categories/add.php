<?php
/**
 * VIBEDAYBKK Admin - Add Article Category
 */
...
// Permission check
require_permission('article_categories', 'create');

$page_title = 'เพิ่มหมวดหมู่บทความ';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (!empty($data['slug'])) {
            $stmt = $conn->prepare("SELECT id FROM article_categories WHERE slug = ?");
        ...
        if (empty($errors)) {
            if (db_insert($conn, 'article_categories', $data)) {
                $category_id = $conn->insert_id;
                log_activity($conn, $_SESSION['user_id'], 'create', 'article_categories', $category_id);
                set_message('success', 'เพิ่มหมวดหมู่บทความสำเร็จ');
...
<div class="flex items-center justify-between mb-6">
    <h2 class="text-3xl font-bold text-gray-900 flex items-center">
        <i class="fas fa-plus-circle mr-3 text-green-600"></i>เพิ่มหมวดหมู่บทความ
    </h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" required 
                           ...
                           placeholder="เช่น แฟชั่น, เทคโนโลยี, อาหาร">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                    ...
                    <input type="text" name="slug" required 
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200"
                           placeholder="fashion, technology, food">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="flex items-center space-x-3">
                        <input type="text" name="icon" id="icon-input" value="fa-folder" 
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <input type="text" name="color" value="from-gray-500 to-gray-700" 
