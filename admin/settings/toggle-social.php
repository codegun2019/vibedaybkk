...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $settingKey = "social_{$platform}_enabled";
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            log_activity($conn, $_SESSION['user_id'], 'update', 'settings', 0, "Toggle {$platform}: " . ($enabled ? 'enabled' : 'disabled'));
            
            $response['success'] = true;
            $response['status'] = $enabled ? 'active' : 'inactive';
            $response['message'] = $enabled ? 'เปิดใช้งานเรียบร้อยแล้ว' : 'ปิดใช้งานเรียบร้อยแล้ว';
        } else {
